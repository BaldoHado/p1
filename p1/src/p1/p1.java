...
public class p1 {
	...
	public static void main(String[] args) {
		Scanner scanner;
			...
			if (li.indexOf("--Incoordinate true") >= 0) {
				f = new File("adamMap.txt");
			}
			if (li.indexOf("--Outcoordinate true") >= 0) {
				f = new File("adamCoordinateMap");
			}
			if (li.indexOf("--Help true") >= 0 || li.indexOf("--Help") >= 0) {
				System.out.println("--Stack true : Runs path program with Stacks");
				System.out.println("--Queue true : Runs path program with Queues");
				System.out.println("--Opt true : Runs optimal path");
				System.out.println("--Time true : Returns runtime");
				System.out.println("--Incoordinate true : Uses a map (no points)");
				System.out.println("--Outcoordinate true : Uses a map of points (coordinates)");
			}
			...
			try {
				...
				if (li.indexOf("--Stack true") >= 0) {
					coordinateBasedStacks(scanner);
				}
				if (li.indexOf("--Queue true") >= 0) {
					coordinateBased(scanner);
				}
				if (li.indexOf("--Opt true") >= 0) {
					coordinateBased(scanner);
				}
				if (li.indexOf("--Time true") >= 0) {
...
//				}
	...
	public static void findCakeQueue(Coord c) {
		...
		if (curRow > (amtDoors*rows-1) && curCol > 0 && curRow < ((amtDoors+1)*rows-1) && curRow > 0 && curCol < lines[0].length-1 && lines[curRow][curCol] != "C".charAt(0) && lines[curRow][curCol] != "|".charAt(0)) {
